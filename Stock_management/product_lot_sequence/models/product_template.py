...
class ProductTemplate(models.Model):
    ...
    lot_sequence_code = fields.Char('Sequence Code')
    lot_sequence_name = fields.Char('Sequence Name')
    
    
    @api.model
    def _create_lot_sequence(self, vals):
        """ Create new no_gap entry sequence"""
        name = vals.get("lot_sequence_name", False) or self.lot_sequence_name
        sequence_code = vals.get("lot_sequence_code", False) or self.lot_sequence_code
        ...
        for template in self:
            ...
            if tracking in ["lot", "serial"]:
                if not vals.get("lot_sequence_id", False):
                    vals["lot_sequence_id"] = (template.sudo()._create_lot_sequence(vals).id)
                else:
